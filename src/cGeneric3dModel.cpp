...
cGeneric3dModel::cGeneric3dModel(cCellMesh *m, cPetscSolver *s) {
	mesh = m;
	solver = s;
	...
	init_u();
	solver->init(VARIABLES * mesh->nodes_count, u, Amat);
}
...
void cGeneric3dModel::get_parameters(){
    ...
    while(getline(model_file, line)){
		...
		if((n + tokens.size()) > PCOUNT) solver->fatal_error("too many parameters in the model parameters file");
		for(unsigned int m = 0; m < tokens.size(); m++) p[n++] = atof(tokens[m].c_str());
    }
	model_file.close();
	if(n != PCOUNT) solver->fatal_error("too few parameters in the model parameters file");
	numt = p[tend] / p[delt];
}
